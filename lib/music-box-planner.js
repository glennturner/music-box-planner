...
MusicBoxPlanner.prototype._injectCalendar = function () {
  ...
  let showtimeDateEles = document.querySelectorAll(`.${this._showtimesBlockClass} .${this._showtimesDateClass}`)
...
MusicBoxPlanner.prototype._getTicketUrlFromBlock = function (showtime) {
  return showtime.querySelectorAll(`.${this._showtimesBlockClass} a`)[0].href
}
    ...
    ((resp) => {
      ...
      if (eles.length) {
        ...
        if (remainingSeats) {
          ...
          showtimeEle.querySelector('.showtime .purchase').innerText = 'Unknown!'
        }
      } else {
