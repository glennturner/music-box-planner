...
MusicBoxPlanner.prototype._getDateFromPicker = function() {
  ...
  console.log(picker.selectedOptions[0].value)
...
MusicBoxPlanner.prototype._getCalendarDateFromURL = function () {
  console.log('CALENDAR DATE? ' + this._isCalendar())
  console.log()
  return window.location.pathname.split('/').splice(2,3)
}

MusicBoxPlanner.prototype._injectCalendar = function () {
  const [ month, year ] = this._getCalendarDateFromURL()
  ...
  */
  ...
  const calendarWDay = parsedDate.getDay().wdayName();
  calendarHTML = calendarWDay + calendarHTML;
