...
MusicBoxPlanner.prototype._isCalendar = function () {
...
/*
  ...
  >
    <input type="month"
      ...
      class="${this._showtimesCalendarClass} form-control"
      value="${this.calendarDateNum.toLocaleDateString('en-CA', calOpts)}"
      min="2021-01"
      max="${maxDate.toLocaleDateString('en-CA', calOpts)}"
      pattern="[0-9]{4}-[0-9]{2}"
      data-set-date="${this.calendarDateNum.toLocaleDateString('en-CA', calOpts)}"
      required
    />
...
*/
  let calendarHTML = `
      <select
        name="${this._showtimesCalendarClass}"
        class="${this._showtimesCalendarClass} form-control form-select"
      >${this._monthPickerOptions(minDate, maxDate, this.calendarDateNum)}
      </select>
  `

  let calendarCont = document.createElement('span')
