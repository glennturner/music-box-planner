MusicBoxPlanner = function () {
  ...
  this._showtimeSelectors = '#showtimes h3 a, .programming-showtimes'
  ...
  this._showtimesCalendarBlockClass = 'calendar-cell:not(.empty, .calendar-head)'
  this._showtimesBlockClass = 'view-showtime-blocks'
...
MusicBoxPlanner.prototype.displayPlanner = function () {
  ...
  if (this._mb.isCalendarPage()) {
    ...
    this._injectCalendarShowtimeExtras()
  }
...
MusicBoxPlanner.prototype._generateShowtimeId = function (data) {
  const url = new URL(data.ticketUrl)
  return 'showtime-' + url.pathname.split('/')[3]
}
...
MusicBoxPlanner.prototype._monthPickerOptions = function(
  start, end, current = new Date
) {
...
MusicBoxPlanner.prototype._injectShowtimeExtras = function () {
  ...
  // Really hacky way to manage multi-showtime blocks.
  let data
  showtimes.forEach(showtime => {
    // Ignore past events
    if (!showtime.classList.contains('past')) {
      switch (this._mb.pageType()) {
        case 'film':
          data = this._getDataFromPage(showtime)
          break
        case 'calendar':
          break
        default:
...
MusicBoxPlanner.prototype._injectCalendarShowtimeExtras = function () {
  let days = this._getFutureShowtimesEles()

  days.forEach(block => {
    let showtimes = this._getDataFromCalendarDayBlock(block)
    showtimes.forEach(showtime => {
      let showtimeEle = document.getElementById(showtime.id)
      let gcalEle = this._constructGCalEle(showtime)

      document.getElementById(showtime.id).after(showtimeEle, gcalEle)
    })
  })
}

MusicBoxPlanner.prototype._constructGCalEle = function (data, opts={
    linkType: 'icon'
  }
) {
...
MusicBoxPlanner.prototype._getDataFromCalendarDayBlock = function (block) {
  let showtimes = []

  const calDateEle = block.querySelector('.calendar-date h2')
  calDateEle.removeChild(calDateEle.querySelector('.d-md-none'))
  const calDateStr = calDateEle.textContent

  // Get showtimes.
  block.querySelectorAll('.programming-content').forEach(film => {
    const title = this._getTitleFromBlock(film)
    const eventUrl = this._getEventUrlFromBlock(film)

    // Get enabled showtimes.
    film.querySelectorAll('.times a:not(.disabled)').forEach(ele => {
      const [ hour, min, sec ] = formatMusicBoxTimeStr(
        `${calDateStr} ${ele.textContent}`
      ).split(':')

      let date = new Date(Date.parse(calDateStr))
      date.setHours(hour)
      date.setMinutes(min)
      date.setSeconds(sec)
      ...
      showtime.id = this._generateShowtimeId(showtime)
      ele.id = showtime.id
      ele.classList.add('showtime-purchase-ticket')
...
MusicBoxPlanner.prototype._getDataFromShowtimeBlock = function (showtime) {
  const title = this._getTitleFromBlock(showtime)

  // If no title, there is no showtime.
  if (!title) { return }
...
MusicBoxPlanner.prototype._getTitleFromBlock = function (block) {
  const titleEle = block.querySelector('h3 a')
...
MusicBoxPlanner.prototype._getStartTimeFromBlock = function (showtime) {
  let currentDateTimeEle = showtime.querySelector('h4')
  let dateStr = currentDateTimeEle.querySelector('span').textContent

  const d = new Date
  const screeningYear = (
    new Date(Date.parse(dateStr)).getMonth() < d.getMonth() ?
      d.getYear() + 1 : d.getYear()
  );
  dateStr += ` ${screeningYear + 1900}`

  const timeStr = formatMusicBoxTimeStr(currentDateTimeEle.innerText)
  const formatted = Date.parse(`${dateStr} ${timeStr}`)
  const screeningDate = new Date(formatted)
...
MusicBoxPlanner.prototype._hasShowtimes = function () {
  return document.querySelectorAll(this._showtimeSelectors).length > 0
}
...
// Watch showtime blocks for changes.
// If so, we assume that the date has changed.
